...
folder=[];
windo = Tk()
windo.configure(background='gray')
windo.title("Efficient Scanner App")        
width = windo.winfo_screenwidth()
height = windo.winfo_screenheight()
windo.geometry(f'{width}x{height}') #x=1366,y=768
...
w = 385;h = 535
size = (w, h)
# adjusting frame of app 
header = Frame(windo, width=width*(2/3), height=height/10, bg="white")
header.grid(sticky=N,row=0,column=0)

fm = Frame(windo,  width=width*(1/3), height=height,bg="blue")
fm.place(x=width*(2/3),y=0)
# placcing logo 
...
def Input_Box():
    ...
    get_button = Button(master_2, text = "Confirm", command = lambda: pdf_gen(file_name))
...
def pdf_gen(file_name):
    ...
    if launch == False:
        ...
        img.save('./Scanned_PDF/'+word.replace("_","/")+str(pdf_c)+'_'+str(Hour)+'_'+str(Minute)+'_'+str(Second)+'.pdf',save_all=True, append_images=image_list)
...
browse_btn = Button(windo, text='Browse', bg="midnightblue", fg="white", width=12,
               height=1, font=('times', 14, 'italic bold '), activebackground='yellow')
browse_text.set("Browse")
browse_btn.place(x=750, y=33)
