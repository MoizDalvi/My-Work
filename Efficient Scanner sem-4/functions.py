...
def display_logo(url, row, column):
    img = Image.open(url)
    img = img.resize((row,column))
    img = ImageTk.PhotoImage(img)
    img_label = Label(image=img, bg="white")
    img_label.image = img
    img_label.grid(column=0, row=0, rowspan=1,columnspan=1,sticky = W)
...
def display_icon(url, row, column, stick, funct):
    ...
    icon_label.grid(row = row, column = column, sticky = W,columnspan=1,rowspan=1)
